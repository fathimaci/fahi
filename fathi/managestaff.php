...
<nav>
    ...
    <div>
        ...
        if (isset($_POST['submit'])) {
            ...
            // Insert staff data
            $sql = "INSERT INTO `staff`(`Name`, `phoneno`, `emailid`, `username`, `password`, `category`) VALUES ('$name', '$phone_no', '$email_id', '$username', '$password', '$category')";
        ...
        if (isset($_POST['userdel'])) {
            $email = $_POST['userdel'];
            if (!empty($email)) {
                $sql = "DELETE FROM staff WHERE emailid='$email'";
                $data = mysqli_query($conn, $sql);
                $sql1 = "DELETE FROM login WHERE emailid='$email'";
                $data1 = mysqli_query($conn, $sql1);
